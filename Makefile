...
MICROSERVICES=cmd/device-lora-agent

.PHONY: $(MICROSERVICES)

ARCH=$(shell uname -m)

DOCKERS=docker_device_lora_agent_go
.PHONY: $(DOCKERS)
...
GOFLAGS=-ldflags "-s -w -X github.com/edgexfoundry/device-lora-agent-go.Version=$(VERSION) \
...
cmd/device-lora-agent:
	CGO_ENABLED=0 go build -tags "$(ADD_BUILD_TAGS)" $(GOFLAGS) -o ./cmd/device-lora-agent ./cmd
...
docker_device_lora_agent_go:
	docker build \
		--build-arg ADD_BUILD_TAGS=$(ADD_BUILD_TAGS) \
		--label "git_sha=$(GIT_SHA)" \
		-t edgexfoundry/device-lora-agent:$(GIT_SHA) \
		-t edgexfoundry/device-lora-agent:$(VERSION)-dev \
		.
