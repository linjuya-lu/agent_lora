...
WORKDIR /
COPY --from=builder /device-lora-agent-go/Attribution.txt /
COPY --from=builder /device-lora-agent-go/LICENSE /
COPY --from=builder /device-lora-agent-go/cmd /

EXPOSE 59900

ENTRYPOINT ["/device-lora-agent"]
